...
import DisplayBalance from "@/components/Shared/DisplayBalance";
import useGenerateReferralCode from "../hooks/useGenerateReferral";
...
const ReferralModal = () => {
	...
	} = useGetUserReferralCode(walletAddress);
			...
			<Show when={!!walletAddress}>
				<Dialog>
					...
					<DialogContent className="sm:max-w-md">
						<Show when={!!referralCode && !!walletAddress}>
							<HasReferralCodeWithDependencyFunction
								referralCode={referralCode ? referralCode : "no-code"}
								url={url}
								referralEarningInBnb={referralEarningInBnb ? referralEarningInBnb : "0"}
								referralEarningInUsdt={
...
function GenerateReferralForm({
	...
	function onSubmit(e: React.MouseEvent<HTMLButtonElement, MouseEvent>) {
		...
		if (walletAddress) generateReferralCode(referralCode);
	}
		...
		<>
			<DialogHeader>
				...
				<DialogDescription>
					...
					generate a referral link! You will earn 5% for each token purchase from
					your link.
				</DialogDescription>
			</DialogHeader>
		...
		return (
			<>
				<DialogHeader>
					...
					<DialogDescription>
						💡 Tip: You can share this link on social media or send it to your friends
						via email or WhatsApp to earn 5% for each token purchase from your link.
					</DialogDescription>
				</DialogHeader>
